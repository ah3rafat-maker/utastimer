...
function parseCloudRowsFromSettings(settings){
  ...
  if (Array.isArray(settings.examsData)) return settings.examsData;
  if (typeof settings.examsJson === "string" && settings.examsJson.trim()) {
    try {
      const parsed = JSON.parse(settings.examsJson);
...
function initCloudSync(){
  ...
  try {
    ...
    getCloudRef(CLOUD_KEYS.exams).on("value", snap => {
      const rows = snap.val();
      if (Array.isArray(rows)) applyCloudRows(rows, Date.now());
    });
...
function saveCloudExams(rows){
  ...
  try {
    ...
    // نحفظ البيانات في مسارين: /exams وأيضًا داخل /settings/examsJson.
    // السبب: بعض إعدادات Firebase تسمح بتحديث settings فقط؛ وبهذا تظهر التعديلات على كل المتصفحات.
    getCloudRef(CLOUD_KEYS.exams).set(safeRows);
    getCloudRef(CLOUD_KEYS.settings).update({ examsJson: JSON.stringify(safeRows), updatedAt: stamp });
    setCloudStatus("تم حفظ البيانات سحابيًا ومزامنتها.");
  }
  catch (err) { console.error(err); setCloudStatus("تعذر حفظ البيانات سحابيًا. تحقق من قواعد Realtime Database."); }
}
